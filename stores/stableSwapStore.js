...
class Store {
      ...
      const ps = await Promise.all(
        pairs.map(async (pair) => {
          try {
            ...
            pair.balance = BigNumber(balanceOf).div(10**Number(pair.decimals)).toFixed(parseInt(Number(pair.decimals)))
            pair.totalSupply = BigNumber(totalSupply).div(10**Number(pair.decimals)).toFixed(parseInt(Number(pair.decimals)))
            pair.reserve0 = BigNumber(reserves[0]).div(10**Number(pair.token0.decimals)).toFixed(parseInt(Number(pair.token0.decimals)))
            pair.reserve1 = BigNumber(reserves[1]).div(10**Number(pair.token1.decimals)).toFixed(parseInt(Number(pair.token1.decimals)))
            pair.claimable0 = BigNumber(claimable0).div(10**Number(pair.token0.decimals)).toFixed(Number(pair.token0.decimals))
            pair.claimable1 = BigNumber(claimable1).div(10**Number(pair.token1.decimals)).toFixed(Number(pair.token1.decimals))
  ...
  _getDepositAllowance = async (web3, token, account) => {
    try {
      ...
      return BigNumber(allowance).div(10**Number(token.decimals)).toFixed(Number(token.decimals))
    } catch (ex) {
      console.error(ex)
      return null
    }
  ...
  quoteSwap = async (payload) => {
    try {
      ...
      const routerContract = new web3.eth.Contract(CONTRACTS.ROUTER_ABI, CONTRACTS.ROUTER_ADDRESS)
      const sendFromAmount = BigNumber(Number(fromAmount)).times(10**Number(fromAsset.decimals)).toFixed()
      ...
      for(let i = 0; i < receiveAmounts.length; i++) {
        ...
        amountOuts[i].finalValue = BigNumber(receiveAmounts[i][receiveAmounts[i].length-1]).div(10**Number(toAsset.decimals)).toFixed(Number(toAsset.decimals))
      }
