  ...
  _getBaseAssetInfo = async (web3, account) => {
    try {
      ...
      for (let i = 0; i < baseAssets.length; i++) {
        baseAssets[i].balance = BigNumber(baseAssetsBalances[i].balanceOf).div(10 ** Number(BigInt(baseAssets[i].decimals))).toFixed(Number(BigInt(baseAssets[i].decimals)))
        baseAssets[i].isWhitelisted = baseAssetsBalances[i].isWhitelisted
      }
      //console.log(baseAssets[i].balance)
